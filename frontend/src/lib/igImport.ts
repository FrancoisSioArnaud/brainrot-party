...
function collectParticipants(json: unknown): string[] {
...
function collectSharesFromMessages(
  json: unknown,
  out: IgImportShare[],
  rejected: IgRejected[],
  file_name: string,
  ...
  for (const message of messages) {
    ...
    if (sender_name?.trim()) participants.add(sender_name.trim());
...
export async function importInstagramJsonFiles(files: File[]): Promise<IgImportResult> {
  ...
  for (const f of files) {
    ...
    for (const name of collectParticipants(json)) participants.add(name);
