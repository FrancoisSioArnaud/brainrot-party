  ...
  app.get("/ws", { websocket: true }, (conn: any, _req) => {
    ...
    ws.on("message", async (raw: WebSocket.RawData) => {
      ...
      if (msg.type === "START_GAME") {
        ...
        if (cur) {
          broadcast(room_code, { type: "NEW_ITEM", payload: { room_code, round_id: cur.round.round_id, item_id: cur.item.item_id } });
        }
      ...
      if (msg.type === "SUBMIT_VOTE") {
        ...
        // must be expected voter
        if (!state.game.expected_player_ids.includes(myPid)) return;

        // must still be claimed by this device
        ...
        if (!Array.isArray(selections) || selections.length !== cur.item.k) return;

        const unique = new Set(selections);
        if (unique.size !== selections.length) return;

        const selectable = selectableSenders(state);
        for (const s of selections) if (!selectable.includes(s)) return;
        ...
        send(ws, {
          type: "VOTE_ACK",
          payload: { room_code, round_id: cur.round.round_id, item_id: cur.item.item_id },
        });

        broadcast(room_code, { type: "PLAYER_VOTED", payload: { room_code, player_id: myPid } });
      ...
      if (msg.type === "END_ITEM") {
        ...
        if (state.game.current_item_index < round.items.length) {
          ...
          broadcast(room_code, { type: "NEW_ITEM", payload: { room_code, round_id: round.round_id, item_id: next.item_id } });
          broadcastState(state, room_code);
          return;
        }
        ...
        broadcast(room_code, { type: "ROUND_RECAP", payload: { room_code, round_id: round.round_id, scores: state.scores } });
        broadcastState(state, room_code);
        return;
      }
      ...
      if (msg.type === "START_NEXT_ROUND") {
        ...
        if (state.game.current_round_index >= state.setup.rounds.length) {
          ...
          broadcast(room_code, { type: "GAME_OVER", payload: { room_code, scores: state.scores } });
          broadcastState(state, room_code);
          return;
        }
        ...
        broadcast(room_code, { type: "ROUND_FINISHED", payload: { room_code, round_id: round.round_id } });

        const first = round.items[0];
        broadcast(room_code, { type: "NEW_ITEM", payload: { room_code, round_id: round.round_id, item_id: first.item_id } });
